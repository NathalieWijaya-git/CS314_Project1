...
static void assign()
{
	...
	next_token();
	if (token != '=') {
		ERROR("Expected '=' for assignment operation\n");
		exit(EXIT_FAILURE);
	}
	next_token();
	...
	CodeGen(STORE, token_temp, right_reg, EMPTY_FIELD);
}
...
static void read()
{
	...
	if (token != '?') {
		ERROR("Expected '?' for read operation\n");
		exit(EXIT_FAILURE);
	}
	next_token();
...
static void print()
{
	...
	if (token != '%') {
		ERROR("Expected '%' for print operation\n");
		exit(EXIT_FAILURE);
	}
	next_token();
...
static void stmt()
{
	...
	case '%':
		...
		break;
	default:
		ERROR("Unexpected token in statement: %c\n", token);
        exit(EXIT_FAILURE);
	}
}
...
static void morestmts()
{
	...
	case ';':
		...
		if (token == 'a' || token == 'b' || token == 'c' || token == 'd' || token == 'e' || token == 'f' || token == '?' || token == '%') {
			stmt();
			morestmts();
		} else if (token != '!') {
			ERROR("Unexpected token after ';' in morestmts: %c\n", token);
			exit(EXIT_FAILURE);
		}
		break;
	case '!':
		return;
	default:
		ERROR("Expected ';' for morestmts operation\n");
		exit(EXIT_FAILURE);
	}
}
...
static void stmtlist()
{
	...
	if (token == 'a' || token == 'b' || token == 'c' || token == 'd' || token == 'e' || token == 'f' || token == '?' || token == '%') {
        ...
        ERROR("Unexpected token at stmtlist: %c\n", token);
        exit(EXIT_FAILURE);
	}
}
